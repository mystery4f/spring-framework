...
public class AnnotationConfigWebApplicationContext extends AbstractRefreshableWebApplicationContext
	...
	/**
	 * 注册通过{@link #register(Class...)}指定的类的{@link org.springframework.beans.factory.config.BeanDefinition}，
	 * 并扫描通过{@link #scan(String...)}指定的包。
	 * <p>对于通过{@link #setConfigLocation(String)}或{@link #setConfigLocations(String[])}指定的任何值，
	 * 首先尝试将每个位置加载为类，如果类加载成功，则注册一个{@code BeanDefinition}，
	 * 如果类加载失败（即抛出{@code ClassNotFoundException}），则假定该值是一个包，
	 * 并尝试扫描该包以查找组件类。
	 * <p>启用默认的注解配置后处理器集，使得可以使用{@code @Autowired}、{@code @Required}等注解。
	 * <p>配置类的bean定义使用生成的bean定义名称进行注册，除非在stereotype注解中提供了{@code value}属性。
	 *
