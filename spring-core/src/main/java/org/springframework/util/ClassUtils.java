...
public abstract class ClassUtils {
	...
	/**
	 * 检查右侧类型是否可以分配给左侧类型，假设通过反射设置。将基本包装类视为可分配给相应的基本类型。
	 *
	 * @param lhsType 目标类型
	 * @param rhsType 应分配给目标类型的值类型
	 * @return 如果目标类型可以从值类型分配
	 * @see TypeUtils#isAssignable(java.lang.reflect.Type, java.lang.reflect.Type)
	 */
	public static boolean isAssignable(Class<?> lhsType, Class<?> rhsType) {
		// 断言lhsType和rhsType不能为空
		Assert.notNull(lhsType, "Left-hand side type must not be null");
		Assert.notNull(rhsType, "Right-hand side type must not be null");
		// 如果lhsType能赋值给rhsType，则返回true
		if (lhsType.isAssignableFrom(rhsType)) {
			return true;
		}
		// 如果lhsType是基本类型，则查找对应的基本类型包装类
		if (lhsType.isPrimitive()) {
			Class<?> resolvedPrimitive = primitiveWrapperTypeMap.get(rhsType);
			// 如果查找到的基本类型包装类与lhsType相同，则返回true
			return (lhsType == resolvedPrimitive);
		}
		// 如果rhsType是基本类型，则查找对应的基本类型包装类
		else {
			Class<?> resolvedWrapper = primitiveTypeToWrapperMap.get(rhsType);
			// 如果查找到的基本类型包装类能赋值给lhsType，则返回true
