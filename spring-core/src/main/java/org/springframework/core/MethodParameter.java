...
/**
 * 辅助类，封装了方法参数的规范，即一个{@link Method}或{@link Constructor}以及一个参数索引和一个声明的泛型类型的嵌套类型索引。作为一个传递的规范对象很有用。
 *
 * <p>从4.2版本开始，有一个{@link org.springframework.core.annotation.SynthesizingMethodParameter}子类可用，它合成了带有属性别名的注解。该子类用于Web和消息端点处理，特别是。
 *
...
public class MethodParameter {
	...
	public Type getGenericParameterType() {
		...
		if (paramType == null) {
			if (this.parameterIndex < 0) {
				Method method = getMethod();
				paramType = (method != null ? (KotlinDetector.isKotlinReflectPresent() && KotlinDetector.isKotlinType(
						getContainingClass()) ? KotlinDelegate.getGenericReturnType(method) : method.getGenericReturnType()) : void.class);
			} else {
	...
	public boolean equals(@Nullable Object other) {
		...
		return (getContainingClass() == otherParam.getContainingClass() && ObjectUtils.nullSafeEquals(this.typeIndexesPerLevel,
