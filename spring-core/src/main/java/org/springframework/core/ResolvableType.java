...
public class ResolvableType implements Serializable {
	...
	/**
	 * 解析此类型为{@link java.lang.Class}，如果无法解析则返回{@code null}。如果直接解析失败，此方法将考虑{@link TypeVariable TypeVariables}和{@link WildcardType WildcardTypes}的边界；但将忽略{@code Object.class}的边界。
	 * <p>如果此方法返回非空的{@code Class}且{@link #hasGenerics()}返回{@code false}，则给定类型有效包装了一个普通的{@code Class}，如果需要，可以进行普通的{@code Class}处理。
