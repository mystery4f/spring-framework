...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
	...
	public void setWrappedInstance(Object object, @Nullable String nestedPath, @Nullable Object rootObject) {
		...
		// 设置根对象，如果嵌套路径不为空，则使用嵌套路径上的对象，否则使用实际对象
	...
	public void setPropertyValue(PropertyValue pv) throws BeansException {
		...
		if (tokens == null) {
			...
			// 如果嵌套属性访问器等于当前属性访问器，则将其 resolvedTokens 设置为 tokens
			if (nestedPa == this) {
	...
	private void processKeyedProperty(PropertyTokenHolder tokens, PropertyValue pv) {
		...
		if (propValue.getClass().isArray()) {
			...
			try {
				...
				// 如果数组索引大于数组长度，且数组索引小于自动增长集合的限制
				if (arrayIndex >= length && arrayIndex < this.autoGrowCollectionLimit) {
					// 获取数组组件类型
					Class<?> componentType = propValue.getClass().getComponentType();
					// 创建一个新的数组，并将原始数组中的值复制到新数组中
			...
			// 获取值的类型描述符
			Object convertedMapValue = convertIfNecessary(tokens.canonicalName,
					oldValue,
					pv.getValue(),
					mapValueType,
					ph.nested(tokens.keys.length)
	...
	private Object getPropertyHoldingValue(PropertyTokenHolder tokens) {
		...
		Assert.state(tokens.keys != null, "No token keys");
		// Create a new PropertyTokenHolder object with the actual name of the token
		PropertyTokenHolder getterTokens = new PropertyTokenHolder(tokens.actualName);
		// Set the canonical name of the token
		getterTokens.canonicalName = tokens.canonicalName;
		// Create a new String array with the length of the tokens.keys array minus 1
		getterTokens.keys = new String[tokens.keys.length - 1];
		// Copy the tokens.keys array to the new String array
		System.arraycopy(tokens.keys, 0, getterTokens.keys, 0, tokens.keys.length - 1);

		// Fetch the property value for all keys but the last one
		Object propValue;
		try {
			propValue = getPropertyValue(getterTokens);
		} catch (NotReadablePropertyException ex) {
			// Throw an exception if the property cannot be read
			throw new NotWritablePropertyException(getRootClass(),
					this.nestedPath + tokens.canonicalName,
		...
		if (propValue == null) {
			...
			if (isAutoGrowNestedPaths()) {
				// Get the index of the last '[' character in the canonical name
				int lastKeyIndex = tokens.canonicalName.lastIndexOf('[');
				// Set the canonical name to everything before the last '[' character
				getterTokens.canonicalName = tokens.canonicalName.substring(0, lastKeyIndex);
				// Set the property value to the default value
				propValue = setDefaultValue(getterTokens);
			} else {
				// Throw an exception if the property value is null and autoGrowNestedPaths is false
				...
				if (pv.isConverted()) {
					...
					if (isExtractOldValueForEditor() && ph.isReadable()) {
						try {
							...
							if (logger.isDebugEnabled()) {
								logger.debug(
										"Could not read previous value of property '" + this.nestedPath + tokens.canonicalName + "'",
										ex
								);
							}
						}
	...
	private PropertyTokenHolder getPropertyNameTokens(String propertyName) {
		...
		if (!keys.isEmpty()) {
			tokens.canonicalName += PROPERTY_KEY_PREFIX + StringUtils.collectionToDelimitedString(
					keys,
